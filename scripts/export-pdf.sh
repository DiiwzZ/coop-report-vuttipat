...
# Resolve output to absolute path (must happen before any cd)
...
// Open with ?export=1 so the deck enters export mode
await page.goto(`http://localhost:${port}/?export=1`, { waitUntil: 'networkidle' });
...
for (let i = 0; i < slideCount; i++) {
  // waitForSlideReady: switches slide, waits for fonts + images + settle delay
  await page.evaluate((index) => window.waitForSlideReady(index), i);

  // Small extra buffer after the in-page promise resolves
